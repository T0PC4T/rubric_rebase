        ...
        RubricToolbarDropdown(
          onUpdate: (value) {
            if (value case double newValue) {
              editorState.canvas.updateProperties<DividerElementModel>(
                element,
                (properties) => properties.copyWith(weight: newValue).toJson(),
              );
            }
          },
          items: [for (var i = 0; i <= 5; i++) RubricDropdownMenuItem(value: i, text: "${i}px")],
          child: Row(
            spacing: RubricEditorStyle.paddingUnit * 0.5,
            children: [
              RubricIcon(Icons.line_weight, size: ElementToolbarWidget.iconSize),
