...
class TextEditorElement extends StatefulWidget {
  ...
  const TextEditorElement.header({super.key, required this.element}) : header = true;

  @override
  State<TextEditorElement> createState() => TextEditorElementState();
}

class TextEditorElementState<T extends TextEditorElement> extends FocusableState<T> {
  ...
  void onFocus(bool focused) {
    if (focused) {
      ...
      if (_changed) {
        editorState.canvas.updateProperties<TextElementBaseModel>(widget.element, (properties) {
          return switch (properties) {
            TextElementModel textElement => textElement.copyWith(text: controller.text).toJson(),
            LinkElementModel linkElement => linkElement.copyWith(text: controller.text).toJson(),
  ...
  void didUpdateWidget(TextEditorElement oldWidget) {
    ...
    final oldProperties = oldWidget.element.getProperties<TextElementBaseModel>();
  ...
  Widget build(BuildContext context) {
    ...
    return isFocused
        ...
        : TextReaderWidget(element: element, ifEmpty: "Insert Text Here...", canvas: editorState.canvas.value);
  }
}
...
class TextReaderWidget extends StatelessWidget {
  ...
  const TextReaderWidget({super.key, required this.element, this.ifEmpty, required this.canvas});
  ...
  List<TextSpan> _parseText(String rawText, TextStyle baseStyle) {
    ...
    if (firstMatchIndex > 0) {
      spans.add(TextSpan(text: rawText.substring(0, firstMatchIndex), style: baseStyle));
    }
    ...
    return SizedBox(
      ...
      child: RichText(
        ...
        text: TextSpan(
          ...
          children: _parseText(
            (ifEmpty != null && properties.text.isEmpty) ? "Write something here..." : properties.text,
            baseStyle,
          ),
        ),
      ),
